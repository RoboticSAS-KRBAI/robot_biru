...
from robotic_sas_auv_ros.msg import Error, Actuator, IsStable, ObjectDifference,Manual
import numpy as np
...
<<<<<<< HEAD
    def thruster(self, pwm_thruster_1, pwm_thruster_2, pwm_thruster_3, pwm_thruster_4, pwm_thruster_5, pwm_thruster_6, pwm_thruster_7, pwm_thruster_8, pwm_thruster_9, pwm_thruster_10):
        self.pwm_actuator.thruster_1 = pwm_thruster_1
        self.pwm_actuator.thruster_2 = pwm_thruster_2
        self.pwm_actuator.thruster_3 = pwm_thruster_3
        self.pwm_actuator.thruster_4 = pwm_thruster_4
...
<<<<<<< HEAD
        self.thruster_controller = ThrusterController(1)
=======
>>>>>>> 1d758f689e6c1a54cb7ea24def456dd31db67e8b
...
<<<<<<< HEAD
        self.thruster_value = [0,0,0,0,0,0,0,0]
        self.offset_thruster_value = [0,0,0,0,0,0,0,0]
...
>>>>>>> 1d758f689e6c1a54cb7ea24def456dd31db67e8b
        ...
        rospy.Subscriber('/manual_control', Manual, self.callback_manual)

        self.pub_dive = rospy.Publisher('dive',Bool,queue_size=10)

    def callback_manual(self, data: Manual):
        self.manual.fx = data.fx
        self.manual.fy = data.fy
        self.manual.tau = data.tau
...
<<<<<<< HEAD
        self.offset_thruster_value[0] = self.thruster_value[0]
        self.offset_thruster_value[1] = self.thruster_value[1]
        self.offset_thruster_value[2] = self.thruster_value[2]
        self.offset_thruster_value[3] = self.thruster_value[3]
        self.offset_thruster_value[4] = self.thruster_value[4]
        self.offset_thruster_value[5] = self.thruster_value[5]
        self.offset_thruster_value[6] = self.thruster_value[6]
        self.offset_thruster_value[7] = self.thruster_value[7]
...
>>>>>>> 1d758f689e6c1a54cb7ea24def456dd31db67e8b
        ...
        pwm_thruster_4 = 1500
<<<<<<< HEAD
        pwm_thruster_5 = self.constrain(1500 + (self.thruster_value[6] - self.get_offset(self.offset_thruster_value[6])), min_pwm, max_pwm)
        pwm_thruster_6 = self.constrain(1500 - (self.thruster_value[5] - self.get_offset(self.offset_thruster_value[5])), min_pwm, max_pwm)
        pwm_thruster_7 = self.constrain(1500 + (self.thruster_value[4] - self.get_offset(self.offset_thruster_value[4])), min_pwm, max_pwm)
        pwm_thruster_8 = self.constrain(1500 - (self.thruster_value[7] - self.get_offset(self.offset_thruster_value[7])), min_pwm, max_pwm)
        pwm_thruster_9 = 1500
        pwm_thruster_10 = 1500

        self.movement.thruster(pwm_thruster_1, pwm_thruster_2, pwm_thruster_3, pwm_thruster_4, pwm_thruster_5, pwm_thruster_6, pwm_thruster_7, pwm_thruster_8, pwm_thruster_9, pwm_thruster_10)

    def thruster_movement(self):
        min_pwm = 1000
        max_pwm = 2000

        pwm_thruster_1 = self.constrain(1500 - (self.thruster_value[1] * 500 - self.get_offset(self.offset_thruster_value[1])), min_pwm, max_pwm)
        pwm_thruster_2 = self.constrain(1500 - (self.thruster_value[0] * 500 - self.get_offset(self.offset_thruster_value[0])), min_pwm, max_pwm)
        pwm_thruster_3 = self.constrain(1500 - (self.thruster_value[3] * 500 - self.get_offset(self.offset_thruster_value[3])), min_pwm, max_pwm)
        pwm_thruster_4 = self.constrain(1500 - (self.thruster_value[2] * 500 - self.get_offset(self.offset_thruster_value[2])), min_pwm, max_pwm)
        pwm_thruster_5 = self.constrain(1500 + (self.thruster_value[6] - self.get_offset(self.offset_thruster_value[6])), min_pwm, max_pwm)
        pwm_thruster_6 = self.constrain(1500 - (self.thruster_value[5] - self.get_offset(self.offset_thruster_value[5])), min_pwm, max_pwm)
        pwm_thruster_7 = self.constrain(1500 + (self.thruster_value[4] - self.get_offset(self.offset_thruster_value[4])), min_pwm, max_pwm)
        pwm_thruster_8 = self.constrain(1500 - (self.thruster_value[7] - self.get_offset(self.offset_thruster_value[7])), min_pwm, max_pwm)
    ...
    def stabilize_thruster_yaw(self, error_depth, error_yaw, error_roll, error_pitch):
        self.t_yaw = np.interp(self.pid_yaw(error_yaw), [-500, 500], [-3, 3])
        self.thruster_value = self.thruster_controller.compute_thruster_forces(0, 0, self.pid_depth(error_depth), self.t_yaw, -(self.pid_roll(error_roll)), self.pid_pitch(error_pitch))

    def stabilize_thruster_camera(self, error_depth, error_yaw, error_roll, error_pitch):
        self.t_yaw = np.interp(self.pid_camera(error_yaw), [-500, 500], [-3, 3])
        self.thruster_value = self.thruster_controller.compute_thruster_forces(0, 0, self.pid_depth(error_depth), self.t_yaw, -(self.pid_roll(error_roll)), self.pid_pitch(error_pitch))

    def stabilize_thruster_manual(self, error_depth, error_roll, error_pitch):
        self.thruster_value = self.thruster_controller.compute_thruster_forces(self.manual.fx, self.manual.fy, self.pid_depth(error_depth), self.manual.tau, -(self.pid_roll(error_roll)), self.pid_pitch(error_pitch))
    ...
    # def stabilize_manual(self, error):
    #     self.t_yaw = np.interp(self.pid_yaw(error), [-500, 500], [-3, 3])
    #     self.thrust_surge_sway_yaw = self.ssyController.control(self.manual.fx, self.manual.fy, self.manual.tau)
=======
        pwm_thruster_9 = 1500
        pwm_thruster_10 = 1500
        self.movement.go_surface(pwm_thruster_1, pwm_thruster_2, pwm_thruster_3, pwm_thruster_4, pwm_thruster_9, pwm_thruster_10)
    ...
    def surge_yaw(self):
        ...
        pwm_thruster_3 = self.constrain(1500 - (self.thrust_surge_sway_yaw[3] * 500 - self.get_offset(self.offset_surge_sway_yaw[3])), min_pwm, max_pwm)
        pwm_thruster_4 = self.constrain(1500 - (self.thrust_surge_sway_yaw[2] * 500 - self.get_offset(self.offset_surge_sway_yaw[2])), min_pwm, max_pwm)
        pwm_thruster_9 = self.constrain(1500 - (self.thrust_surge_sway_yaw[1] * 500 - self.get_offset(self.offset_surge_sway_yaw[1])), 1450, 1550)
        pwm_thruster_10 = self.constrain(1500 - (self.thrust_surge_sway_yaw[0] * 500 - self.get_offset(self.offset_surge_sway_yaw[0])), 1450, 1550)
...
<<<<<<< HEAD
        ...
        if self.move == "forward":
            self.stabilize_thruster_yaw(data.depth, data.yaw, data.roll, data.pitch)
        elif self.move == "manual":
            self.stabilize_thruster_manual(data.depth, data.roll, data.pitch)
        elif self.move == "camera":
            self.stabilize_surge_yaw_camera(data.depth, self.object_difference.x_difference, data.roll, data.pitch)
        ...
        # elif self.move == "manual":
        #     self.stabilize_manual(data.yaw)
=======
        self.stabilize_depth_pitch_roll(data.depth, data.pitch, data.roll)
        if self.move == "left":
            self.stabilize_sway_yaw_left(data.yaw)
...
<<<<<<< HEAD
        ...
        if self.is_in_range(4, None):
            self.thruster_movement()
        # if self.move == "forward" or self.move == "camera" or self.move == "last" or self.move == "manual":
        #     #print("Surge Yaw")
        #     self.surge_yaw()
